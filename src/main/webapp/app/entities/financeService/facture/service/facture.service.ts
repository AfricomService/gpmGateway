...
type RestOf<T extends IFacture | NewFacture> = Omit<T, 'dateFacture' | 'dateEcheance' | 'dateDecharge' | 'createdAt' | 'updatedAt'> & {
  dateFacture?: string | null;
  dateEcheance?: string | null;
  dateDecharge?: string | null;
...
export class FactureService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/factures', 'financeservice');
  ...
  protected convertDateFromClient<T extends IFacture | NewFacture | PartialUpdateFacture>(facture: T): RestOf<T> {
    return {
      ...
      dateDecharge: facture.dateDecharge?.format(DATE_FORMAT) ?? null,
      createdAt: facture.createdAt?.toJSON() ?? null,
      updatedAt: facture.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFacture: RestFacture): IFacture {
    return {
      ...
      dateDecharge: restFacture.dateDecharge ? dayjs(restFacture.dateDecharge) : undefined,
      createdAt: restFacture.createdAt ? dayjs(restFacture.createdAt) : undefined,
      updatedAt: restFacture.updatedAt ? dayjs(restFacture.updatedAt) : undefined,
    };
  }
